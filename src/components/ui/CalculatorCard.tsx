...
export function CalculatorCard({
  ...
  return (
    <div
      className={cn(
        "panel-warm relative rounded-3xl border border-wood-200/50 overflow-hidden",
        className
      )}
    >
      <div className={cn("absolute top-0 left-0 right-0 h-1 bg-gradient-to-r", accents[accent])} />
      <div className="p-6 md:p-8 space-y-6">
        <div className="text-center md:text-left">
          <h3 className="font-serif text-xl md:text-2xl font-bold text-wood-950">{title}</h3>
          {description && (
            <p className="text-sm text-stone-500 mt-1.5 leading-relaxed max-w-md mx-auto md:mx-0">
              {description}
            </p>
          )}
        </div>
